...
export const appWithTranslation = (WrappedComponent, configOverride = null) => {
  const AppWithTranslation = props => {
    ...
    let locale = _nextI18Next?.initialLocale ?? props?.router?.locale ?? null; // Memoize the instance and only re-initialize when either:
    ...
    const i18n = useMemo(() => {
      ...
      let userConfig = configOverride ?? _nextI18Next?.userConfig ?? null;
